...
import { LONG_IMAGE_URL, POSTER_NOT_AVAILABLE, TOP_RATED_MOVIES } from './utils/Api';
...
const TopRated = () => {
  ...
  return (
    ...
    <div className='flex flex-wrap items-center justify-center mt-6'>
      {/* {
        addOns.map((movie) => (
          <div className='p-3 w-[31.3%] laptop:w-[15%] desktop:w-[15%]' key={movie.title}>
            <img src={movie.image} width="230" height="345"/>
            <p className='font-medium block whitespace-nowrap text-ellipsis overflow-hidden'>{movie.title}</p>
            <p className='text-sm font-light text-[#CCCCCC]'>{movie.year}</p>
          </div>
        ))
      } */}
      {
